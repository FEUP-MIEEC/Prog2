...
int heap_insere(vetor *h, compara cmp, char *texto, int prioridade) {
    int i, j;
    if (h->tamanho_actual == h->tamanho_maximo) return 0;
    elemento *e = elemento_cria(prioridade, texto);
    for (i = 0; i < h->tamanho_actual && cmp(h->elementos[i], e) != 1; i++);
    for (j = i; j > 0; j--) h->elementos[j + 1] = h->elementos[j];
    h->elementos[i] = e;
    h->tamanho_actual++;
    return i;
}
