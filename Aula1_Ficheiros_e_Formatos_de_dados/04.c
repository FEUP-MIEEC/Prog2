...
int main(){
	...
	if(!(getc(f)=='B' && getc(f)=='M')){
		...
		// Não é válido! 
		printf("[ERRO] O ficheiro lido não é uma imagem BitMap válida!"); exit(1);
	}
	...
	if (numerodeplanos!=1){
		printf("[ERRO] Este interpretador não está preparado para processar imagens com mais do que um plano!"); exit(2);
	}
	...
	if (nbpp!=24){
		printf("[ERRO] Este interpretador só está preparado para processar imagens com 24 bits por pixel!"); exit(3);
	}
	...
	for(i=altura-1;i>=0;i--){
		for(j=0;j<largura;j++){
			imagem[i][j].b=getc(f); // Lemos para aqui o byte que representa o Blue  (b) deste pixel.
			imagem[i][j].g=getc(f); // Lemos para aqui o byte que representa o Green (g) deste pixel.
			imagem[i][j].r=getc(f); // Lemos para aqui o byte que representa o Red 	 (r) deste pixel.
		}
	}
