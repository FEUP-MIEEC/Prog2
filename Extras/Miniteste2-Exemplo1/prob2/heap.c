...
int menor_que(helemento * e1, helemento * e2);
...
heap* heap_nova(int tamanho_maximo)
{
	...
	h->elementos = (helemento **) calloc(tamanho_maximo, sizeof(helemento *));
...
helemento * elemento_cria(int prioridade, const char * valor)
{
	helemento * elem = (helemento *) malloc(sizeof(helemento));
...
int heap_insere(heap * h, const char * texto, int prioridade)
{
	...
	/* se heap esta' cheia, nao insere helemento */
	...
	/* coloca helemento no fim da heap */
	i = h->tamanho_atual;
	h->elementos[i] = elem;
	h->tamanho_atual++;
	
	/* enquanto helemento for mais prioritario do que o respetivo pai, troca-os */
...
char * heap_remove(heap * h)
{
	...
	/* se heap estiver vazia, nao remove helemento */
	...
	/* coloca ultimo helemento da heap na raiz */
	h->tamanho_atual--;
